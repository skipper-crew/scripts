...
copy_config ()
{
    ...
    echo "mkdir new.irr.ru ..."
    /usr/bin/ssh mephius@$server -p $port "sudo mkdir /usr/local/www/new.irr.ru; sudo chown -R mephius /usr/local/www/new.irr.ru"

    echo "git clone in new.irr.ru ..."
    /usr/bin/ssh mephius@$server -p $port "git clone git@prontosoft.by:irr.ru.git /usr/local/www/new.irr.ru"
    ...
    /usr/bin/ssh mephius@$server -p $port "cd /usr/local/www/new.irr.ru; git checkout master "

    echo "copy server.conf.php ..."
    /usr/bin/ssh mephius@$server -p $port "cp /usr/local/www/irr.ru/config/server.conf.php /usr/local/www/new.irr.ru/config/server.conf.php"

    echo "ln -s img ..."
    /usr/bin/ssh mephius@$server -p $port "ln -s /usr/local/www/img /usr/local/www/new.irr.ru/html/img"

    echo "chmod project folders ..."
    /usr/bin/ssh mephius@$server -p $port "cd /usr/local/www/new.irr.ru; sudo chmod -R 777 cache logs utils/irr.ru/files html/img"

    echo "ln -s xls_files ..."
    /usr/bin/ssh mephius@$server -p $port "ln -s /usr/local/www/xls_files /usr/local/www/new.irr.ru/html/site/files/xls_files"
