...
get_vals() {
    ...
    active_rec=`echo ${out1}|awk '{print $3}'`
...
then
    if [ "$active_rec" -ge "$warning" -a "$active_rec" -lt "$critical" ]
    then
        echo "WARNING - Active Connections: $active_rec | active_con=$active_rec reqpsec=$reqpsec conpsec=$conpsec conpreq=$reqpcon"
	exit $ST_WR
    elif [ "$active_rec" -ge "$critical" ]
    then
        echo "CRITICAL - Active Connections: $active_rec | active_con=$active_rec reqpsec=$reqpsec conpsec=$conpsec conpreq=$reqpcon"
	exit $ST_CR
    else
        echo "OK - Active Connections: $active_rec | active_con=$active_rec reqpsec=$reqpsec conpsec=$conpsec conpreq=$reqpcon"
	exit $ST_OK
    fi
else
    echo "OK - Active Connections: $active_rec"
    exit $ST_OK
fi
